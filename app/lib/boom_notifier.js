 ...
 *
 * The use case for BoomNotifier is for when we wish to run one of these background tasks asyncronously -- for example,
 * sending a bill run would normally be done in the background but we want the 'send bill run' admin endpoint to run it
 * in the foreground so any errors which occur are returned straight away so we don't need to dig through the logs.
 *
 ...
 */
class BoomNotifier extends RequestNotifier {
  omfg (message, data = {}) {
