...
// Thing under test
const { ValidateBillRunInvoiceService } = require('../../app/services')

describe('Validate Bill Run Invoice service', () => {
  let billRun
  ...
  describe('When a valid bill run ID is supplied', () => {
    ...
    describe('and a valid invoice ID', () => {
      it("returns the validated 'invoice'", async () => {
        ...
        const result = await ValidateBillRunInvoiceService.go(billRun.id, invoice.id)
    ...
    describe('and an invalid invoice ID', () => {
      describe('because it is unknown', () => {
        it('throws an error', async () => {
          ...
          const err = await expect(ValidateBillRunInvoiceService.go(billRun.id, unknownInvoiceId)).to.reject()
      ...
      describe('because it is not linked to the bill run', () => {
        it('throws an error', async () => {
          ...
          const err = await expect(ValidateBillRunInvoiceService.go(billRun.id, invoice.id)).to.reject()
